		...
		router.get(RouteConstants.DOWNLOAD_FILE).handler(routingContext -> {
			...
			try {
				...
				if (userRole == null) {
					...
					routingContext.response().putHeader(HttpConstants.HEADER_CONTENT_LENGTH, "" + zipFile.length());
					routingContext.response().putHeader(HttpConstants.HEADER_CONTENT_TYPE, HttpConstants.CONTENT_TYPE_ZIP);
					routingContext.response().putHeader(HttpConstants.HEADER_CONTENT_DISPOSITION, "attachment; filename=\"" + classId
							+ ConfigConstants.HYPHEN + userId + ConfigConstants.ZIP_EXT + "\"");
