...
public final class CassandraClient implements Initializer, Finalizer {
    ...
    private final Map<String, Session> registry = new HashMap<>();
    private final Map<String, String> keySpace = new HashMap<>();
    ...
    private Session initializeDataSource(JsonObject dbConfig) {
        ...
        Cluster archiveCassandraCluster =
            Cluster.builder().withClusterName(dbConfig.getString(ConfigConstants.CASSANDRA_CLUSTER))
    ...
    private void initCassandra(JsonObject dbConfig) {
        try {
            ...
            ConnectionPoolConfigurationImpl poolConfig =
                new ConnectionPoolConfigurationImpl("MyConnectionPool").setPort(9160)
                    .setSeeds(dbConfig.getString(ConfigConstants.CASSANDRA_SEEDS)).setSocketTimeout(30000)
            ...
            AstyanaxContext<Keyspace> logContext =
                new AstyanaxContext.Builder().forCluster(dbConfig.getString(ConfigConstants.CASSANDRA_CLUSTER))
                    .forKeyspace(dbConfig.getString(ConfigConstants.CASSANDRA_KEYSAPCE)).withAstyanaxConfiguration(
                    new AstyanaxConfigurationImpl().setTargetCassandraVersion("2.1.4").setCqlVersion("3.0.0")
            ...
            logContext.start();
            keyspace = logContext.getClient();
            LOGGER.info(
                "Initialized connection to " + dbConfig.getString(ConfigConstants.CASSANDRA_KEYSAPCE) + " keyspace");
        } catch (Exception e) {
